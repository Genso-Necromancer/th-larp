...
func _ready():
	starting_pos = position # = starting_pos
...
func shake():
	...
	var max_shake : int = tex_rext.size.x * 0.09 # 24
...
func hop():
	...
	var hop_height : int = tex_rext.size.x * 0.09 # 24
	var start_pos = tex_rext.position
	var tween = create_tween()
...
func double_hop():
	const TWEEN_DURATION = 0.1
	var hop_height : int = tex_rext.size.x * 0.09 # 30
	var start_pos = tex_rext.position
	var tween = create_tween()
